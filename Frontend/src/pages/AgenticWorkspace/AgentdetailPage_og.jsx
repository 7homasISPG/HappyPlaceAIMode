...
// axios is no longer needed directly here if using the consolidated api.js
...
import { fetchAgentDetails } from '../../api'; // <<< Import from consolidated api.js

// --- Configuration ---
// It's better to get this from environment variables if possible, like in api.js
// Ensure you have VITE_FRONTEND_BASE_URL set in your .env or similar config
...
const SharePanel = ({ agent, onBack }) => {
    ...
    // Ensure agentIdentifier is always a string and correctly retrieved.
    // agent.id is the stringified ObjectId from the backend due to Pydantic's alias.
    const agentIdentifier = agent?.id; 
    const publicApiKey = agent?.public_api_key;
    ...
    return (
        <div className="p-6">
            ...
            <div className="space-y-6">
                ...
                {/* The publicApiKey check here ensures the entire block only renders if the key exists */}
                {publicApiKey && ( 
                    <div>
                        ...
                        <div className="flex gap-2">
                            ...
                            {/* Disable copy button if publicApiKey is not available, though the block itself won't render */}
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                        ...
                        {/* Disable copy button if not ready for links */}
...
// --- Sub-Component: Next Steps Panel ---
...
const AgentDetailPage = ({ agentId, onBack }) => {
    ...
    const renderRightPanel = () => {
        if (isLoadingAgent || !agent) return null; // Ensure agent is loaded before trying to render panels
    ...
    // The `renderRightPanel` handles `!agent` after loading, so this specific check might be redundant if `error` also covers it.
    // Keeping it for clarity in case `!agent` happens without a specific error message.
